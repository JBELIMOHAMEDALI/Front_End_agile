...
export class AdminComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    const payload = {
      id: this.controls.decryptData(idUser),
      tabname: userRole,
      nomId: nom_id,
    };
    const { erorer, msg } = (await this.userServ.getOneUserbyId(
      payload,
      true
    )) as any;
  ...
  redirectToProfile() {
    this.route.navigate(["/dashboard/profil"]);
  }

  getuserRole() {
    return this.controls.decryptData(
      JSON.parse(localStorage.getItem("idConnexion")).type
    );
  }
}
