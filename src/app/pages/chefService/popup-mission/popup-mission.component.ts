...
export class PopupMissionComponent implements OnInit {
  ...
  ngOnInit() {
    this.loadChauffeursNonAffectes((chauffeurs) => {
      ...
      this.chauffeurtoUpdate = chauffeurs.filter(
        (chauffeur) => chauffeur.id_chauffeur != this.mission.id_chauffeur
      );
    });
  ...
  async addMission(form: NgForm) {
    const idChefService = this.controls.decryptData(
      JSON.parse(localStorage.getItem("idConnexion")).idUser
    );
    ...
    try {
      ...
      const { msg, erorer } =
        ((await this.missionService.addMission(mission)) as any) || [];
      if (!erorer) {
      ...
      modelServ.componentInstance.message =
        "Ajout non effectué,Vérifier la voiture affectée à ce chauffeur !";
    }
  }
  ...
  async updateMission(form: NgForm) {
    ...
    try {
      ...
      const { msg, erorer } =
        ((await this.missionService.updateMission(mission)) as any) || [];
      if (!erorer) {
        this.activeModal.dismiss();
        this.controls.reloadComponent();
      }
  ...
  async loadChauffeursNonAffectes(callback) {
    try {
      const { msg, erorer } =
        ((await this.missionService.getChauffeursAffectes()) as any) || [];
      if (!erorer) {
        callback(msg);
      }
    } catch (error) {
  ...
  async supprimerMission() {
    try {
      const { msg, erorer } =
        ((await this.missionService.deleteMission(
          this.id.toString()
        )) as any) || [];
      if (!erorer) {
        this.activeModal.dismiss();
